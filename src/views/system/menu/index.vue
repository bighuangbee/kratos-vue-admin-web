<template>
  <div class="app-container">
    <el-card shadow="always">
      ...
      <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="68px">
        <el-form-item label="菜单名称" prop="menuName">
          <el-input placeholder="菜单名称模糊查询" clearable @keyup.enter="handleQuery" style="width: 240px"
    ...
    <el-card class="box-card">
      ...
      <!--数据表格-->
      <el-table v-loading="loading" :data="menuList" row-key="menuId" border
        ...
        <el-table-column prop="icon" label="分类" align="center" width="100">
          <template #default="scope">
            <el-tag :type="scope.row.menuType === 'M' ? 'danger' : scope.row.menuType === 'C' ? 'success' : 'warning'"
              disable-transitions>{{ typeFormat(scope.row) || '-- --' }}
            </el-tag>
          </template>
        </el-table-column>
...
const getList = () => {
  ...
  listMenu(state.queryParams).then((response: any) => {
    state.menuList = response.data?.list;
    state.loading = false;
  });
};
/** 搜索按钮操作 */
...
onMounted(() => {
  ...
  proxy.getDicts("sys_show_hide").then((response: any) => {
    state.isHideOptions = response.data?.list;
  });
  proxy.getDicts("sys_menu_type").then((response: any) => {
    state.menuTypeOptions = response.data?.list;
  });
  // 查询菜单状态数据字典
